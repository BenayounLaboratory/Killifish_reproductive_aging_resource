...
#Map Male piRNAs to TEs and run LRT DGE
...
#retain only male data (select only columns with male samples)
...
#set prefix
my.outprefix <- paste(Sys.Date(),"Testes_LRT_piRNA",sep="_")
...
write.table(res_LRT_tb ,file = paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"_Testicular_LRT_DGE_piRNA_Table_ALL.txt"), sep = "\t" , row.names = F, quote = F)
write.table(res_LRT_tb.TEs, file = paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"_Testicular_LRT_DGE_Table_piRNATEONLY.txt"), sep = "\t" , row.names = F, quote = F)
...
write.table(tissue.cts, file = paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"_Male_Normalized_DEseq2_piRNATEGenes_Count_Matrix.txt"), sep = "\t" , row.names = T, quote = F)

# Extract significant genes
sig_genes <- tissue.cts[clustering_sig_genes$gene, ]

#retain TEs only (remove all other entries)
sig_genes <- sig_genes[grepl("NotFur", rownames(sig_genes)), ] # 27
...
my.clusters.out <- paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"AGING_grouped_clusters_FDR5.pdf")
...
# write out files
write.table(group1[,1], file = paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"_Male_AGING_Group1_DGE_piRNA_TEs_FDR1e-6.txt"), sep = "\t" , row.names = F, quote = F)
write.table(group2[,1], file = paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"_Male_AGING_Group2_DGE_piRNA_TEs_FDR1e-6.txt"), sep = "\t" , row.names = F, quote = F)
...
write.table(res_LRT.clust, file = paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"_DESeq2_LRT_DGE_Table_ALL_piRNA_TEs_with_DGEPatterns_Clusters_FDR1e-6.txt"), sep = "\t" , row.names = F, quote = F)
...
male_plots <- ggarrange(as.ggplot(heatmap_TE(group1)),
                          as.ggplot(heatmap_TE(group2)))

my.heatmap.out <- paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"AGING_Heatmap_FDR5.pdf")
...
# plot and export
my.heatmap.out <- paste0("./Results/2_Testes_piRNA_DGE/", my.outprefix,"_AGING_Gene_Exp_Correlation_Heatmap.pdf")
pdf(my.heatmap.out)
pheatmap(rbind(correlation,rep(0.9,nrow(correlation))),
         ...
         main = "Testes piRNA Expression Correlation Matrix", 
...
#######################
sink(file = paste("./Results/2_Testes_piRNA_DGE/",my.outprefix,"_session_Info.txt", sep =""))
sessionInfo()
sink()