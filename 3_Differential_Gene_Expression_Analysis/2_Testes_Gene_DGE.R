...
# Male Differential Gene Expression Analysis (LRT)
...
# set prefix
my.outprefix <- paste(Sys.Date(),"Testes_LRT",sep="_")
...
write.table(res_LRT_tb ,file = paste0("./Results/2_Testis_DGE/", my.outprefix,"_Testicular_LRT_DGE_Table_ALL.txt"), sep = "\t" , row.names = F, quote = F)
write.table(res_LRT_tb.genes, file = paste0("./Results/2_Testis_DGE/", my.outprefix,"_Testicular_LRT_DGE_Table_GeneONLY.txt"), sep = "\t" , row.names = F, quote = F)
...
#### FishTEDB TEs all have "NotFur" prefix in name, which helps filter them out
#### They are kept this far to make sure all normalizations take their abundance into account
sigLRT_genes <- sigLRT_genes[!grepl("NotFur", sigLRT_genes$gene), ] #### 603 non TE genes
...
write.table(tissue.cts, file = paste0("./Results/2_Testis_DGE/", my.outprefix, "_Male_Normalized_DEseq2_GeneTE_Count_Matrix.txt"), sep = "\t" , row.names = T, quote=F)
...
#### yield a warning "geom_path: Each group consists of only one observation. Do you need to adjust the group aesthetic?"
...
my.clusters.out <- paste0("./Results/2_Testis_DGE/", my.outprefix,"_AGING_boxplot_grouped_clusters_FDR1e-6.pdf")
...
write.table(group1[,1], file = paste0("./Results/2_Testis_DGE/", my.outprefix,"Male_AGING_Group1_DGE_Genes_FDR1e-6.txt"), sep = "\t" , row.names = F, quote=F)
write.table(group2[,1], file = paste0("./Results/2_Testis_DGE/", my.outprefix,"Male_AGING_Group2_DGE_Genes_FDR1e-6.txt"), sep = "\t" , row.names = F, quote=F)
write.table(group3[,1], file = paste0("./Results/2_Testis_DGE/", my.outprefix,"Male_AGING_Group3_DGE_Genes_FDR1e-6.txt"), sep = "\t" , row.names = F, quote=F)
write.table(group4[,1], file = paste0("./Results/2_Testis_DGE/", my.outprefix,"Male_AGING_Group4_DGE_Genes_FDR1e-6.txt"), sep = "\t" , row.names = F, quote=F)
...
write.table(res_LRT.clust, file = paste0("./Results/2_Testis_DGE/", my.outprefix,"_DESeq2_LRT_DGE_Table_ALL_Genes_with_DGEPatterns_Clusters_FDR1e-6.txt"), sep = "\t" , row.names = F, quote = F)
...
group1_hm <- pheatmap(tissue.cts[group1$genes,],
                      ...
                      main = paste("Genes Up In Middle Age", length(group1$genes), "Genes"), cellwidth = 15)
...
group2_hm <- pheatmap(tissue.cts[group2$genes,],
                      ...
                      show_rownames = F, scale="row",
                      border = NA,cellheight = 0.2,
                      main = paste("Genes Down In Middle Age", length(group2$genes), "Genes"), cellwidth = 15)
...
male_plots <- ggarrange(as.ggplot(group1_hm),
                        as.ggplot(group2_hm),
                        as.ggplot(group3_hm),
                        as.ggplot(group4_hm))

my.heatmap.out <- paste0("./Results/2_Testis_DGE/", my.outprefix,"_AGING_Heatmap_FDR1e-6.pdf")
...
# plot and export
my.heatmap.out <- paste0("./Results/2_Testis_DGE/", my.outprefix,"_AGING_Gene_Exp_Correlation_Heatmap.pdf")
pdf(my.heatmap.out)
pheatmap(rbind(correlation,rep(0.9,nrow(correlation))),
         ...
         main = "Testes Gene Expression Correlation Matrix", 
...
#######################
sink(file = paste("./Results/2_Testis_DGE/", my.outprefix,"_session_Info.txt", sep =""))
sessionInfo()
sink()